...
const CustodySchedule = () => {
  ...
  return (
    <div className="p-4 max-w-md mx-auto" style={{ textAlign: 'center', backgroundColor: 'orange' }}> {/* Adiciona backgroundColor laranja */}
      <h1 className="text-2xl font-bold mb-4">Agenda de Finais de Semana com a Isabela</h1>
      <div className="mb-4">
        <label className="label-grande">Selecione um final de semana no calendário:  </label>
        <input type="date" onChange={handleDateChange} className="w-full p-2 border rounded" />
      </div>
      <div className="bg-blue-100 p-4 rounded" style={{ textAlign: 'center' }}>
        <h2 className="text-xl font-semibold">Responsável no final de semana:</h2>
        <p className="responsavel">{determineParent(selectedDate)}</p>
